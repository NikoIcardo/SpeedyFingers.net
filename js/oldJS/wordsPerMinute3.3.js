...
function init()
{
    ...
    showWord(words); 
    //Listen for typing
    input.addEventListener('input', spacePressed);
...
function unpause()
{
    input.setAttribute('style', 'display: in-line;');
    wpmDisplay.innerHTML = wpm;
    errors.innerHTML = errorCount; 
    message.innerHTML = "<span id = 'highlightColor'>When you are ready, begin typing. <span><br>";
    input.value = ''; 
...
function matchWords()
{
    if(!isPaused)
    {
        if(input.value === words[count] + ' ' || input.value === ' ' + words[count] + ' ')
        {
...
function showWord(words)
{
    ...
    {    
        for (let j = 0; j < scrollSize; j++ ){
            ...
            {
                q = (Math.floor((count - scrollSize)/scrollSize) * scrollSize + j + (i * scrollSize)); // the natural multiples of scrollSize contained in count after scrollSize*2 added to the original q.  
            }
             
            if(q === count)
            {
                shownText += ' ' + "<span style = 'border-radius: 5px;' id = 'highlightColor'>" + words[q] +"</span>"
            }
            else
            {
                if(q < count)
                {
                    shownText += ' ' + "<span style = 'border-radius: 5px; color: gray; '>" + words[q] +"</span>";
                }
                else
                {
                    shownText += ' ' + words[q];
...
function clock()
{
    ...
    else if (wpm > 0)
    {
        // Game is over 
        isPlaying = false;
        //Hide Input to prevent more typing until user is ready 
        input.setAttribute('style','display:none;'); 

        //create button to allow the user to re-enable input when ready
        message.innerHTML = "<span>Press the button below to type again!<span><br><br>" + '<button class = "btn btn-success" onclick = "unpause()" >Type Again</button><br><br>'; // Need to add set of possible responses here...
        
        //add the fbsharestring to message
        fbShareString = '<p>Click the link below to share your score to facebook and challenge your friends!<p><div id="shareBtn" class="btn btn-info clearfix">Share to Facebook</div>';
        message.innerHTML = message.innerHTML + fbShareString;

        //listen for sharebutton 
        document.getElementById("shareBtn").addEventListener('click', share);

        currentWords.innerHTML = "<span style = 'border-radius: 3px; padding: 1px 3px 1px 3px' id = 'highlightColor'> <strong>That was a good run! </strong>";
        ...
        statistics.innerHTML = "<br><strong>Average WPM: " + average.toFixed(2) + '<br>' + " Average Errors: " + averageErrors.toFixed(2) + "<br><br>" + "Error List: </strong>";
...
function errorLogger()
{
    ...
    while(matching)
    {
        ...
        let iChar = input.value[i]; // current letter in input
        ...
        if(iChar != wChar)
        {
            ...
            if (iChar === 'Undefined')
            { 
                error = "You put a space instead of a '" + wChar + "' in the word '" + words[count] + "'<br>Your input: '" + input.value + "'.";
                determined = true;
            }
            while(j < punctuation.length && !determined)
            {
                ...
                {
                    determined = true; 
                    error = "You missed the punctuation here: The '"+ wChar +"' in '" + words[count] + "'<br>Your input: '" + input.value + "'.";
                }
                else
                {
                    j++;
            ...
            if(!determined)
            {
                ...
                while(j < symbols.length && !determined)
                {
                    if(wChar === symbols[j])
                    {
                        ...
                        determined = true; 
                        error = "You missed the symbol here: The '"+ wChar +"' in '"+ words[count] + "'<br>Your input: '" + input.value + "'.";
                    }
                    else
                    {
                        j++;
            ...
            if(!determined)
            {
                ...
                while(j < numbers.length && !determined)
                {
                    ...
                    {
                        determined = true; 
                        error = "You missed the number here: The '"+ wChar +"' in '" + words[count] + "'<br>Your input: '" + input.value + "'.";
                    }
                    else
                    {
                        j++;
            ...
            if(!determined)
            {
                error = "You mispelled: '" + words[count] + "'<br>Your input: '" + input.value + "'.";
            }
            ...
            {
                localStorage.setItem('errors', (localStorage.getItem('errors') + '<br>' + words[count]));
            }
            else 
            {
...
function missCharCount(charM) // charM is the missed character
{
    ...
    while (q < missedCharacters.length)
    {
        ...
        if (charM === undefined || charM === '') // space which is translated to '' from undefined.
        {
            charM = ' ';
        }
...
function lSInit()
{
    ...
    if (localStorage.getItem('Missed Characters') !== null) // any changes to this if must be made in both profile and in wordsPerMinute and in TypeYourOenText.  
    {
...
function share() {
    let shareString = 'I just typed ' + wpmDisplay.innerHTML + ' words in a single minute! Think you can beat my score?'
  FB.ui({
    display: 'popup',
    method: 'share',
