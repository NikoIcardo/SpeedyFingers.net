...
photoIGLink = ['https://www.instagram.com/Stywo/',
...
//Background Image
imageString = "background-image: url('img/backgrounds/" + selection + ".webp');"
localStorage.setItem('Image String',imageString);
image = false; 

function imageBG () // called within Styling
{

    footerText = "<div class = 'row'><div><div style = 'margin: 10px; padding: 5px;'><p id = 'Footer'><a href = 'https://donate.wikimedia.org/w/index.php?title=Special:LandingPage&country=US&uselang=en&utm_medium=donatewiki_page&utm_source=Ways_to_Give&utm_campaign=donate_now_btn'><img src = 'img/wiki.png' height = '75px' width = '75px' style = 'border-radius: 3px; margin-left:5px;' ></a>Most main typing content is sourced from Wikipedia. Please consider donating to maintain its great library of knowledge. <br><br><a id = 'IG'></a></p><p><a class = 'spare' href = 'FBPrivacyandTermsofService.php' style = 'padding:5px;'> &ensp; Click here for the Terms of Service and our Facebook Privacy Policy</a></p></div></div></div>";
    ...
    igLink.innerHTML = "<img src = 'img/ig.png' height = '40px' style = 'border-radius: 3px; margin-left:5px;'>";
    igLink.setAttribute('href', photoIGLink[selection]); 

    if (!image)
    {
        footer.innerHTML =  footer.innerHTML + 'Like this beautiful image from photographer ' + photoCredit[selection] + '? Click the IG link to checkout their portfolio!';
    }
    image = true; 
}
...
{
    //text color
    if(localStorage.getItem('Text Color') !== null && localStorage.getItem('Text Color') !== 'reset' && localStorage.getItem('Text Color') !== '')
    {
    ...
    //Box Color
    if(localStorage.getItem('Box Color') !== null && localStorage.getItem('Box Color') !== 'reset' && localStorage.getItem('Box Color') !== '')
    {
        let BoxColor = 'background-color: '+ localStorage.getItem('Box Color') + ';';
    ...
    {
        localStorage.setItem('Box Color', '#3b4d5e');
        style();
    }

    //Highlight Color
    if(localStorage.getItem('Highlight') !== null && localStorage.getItem('Highlight') !== 'reset')
    {
        if(document.getElementById('highlightColor') !== null)
        {
            document.getElementById('highlightColor').style.backgroundColor = localStorage.getItem('Highlight'); 
        }
    }
    else
    {
        localStorage.setItem('Highlight', '#660033');
    }

    //Background Color
    if(localStorage.getItem('BG Color') !== null && localStorage.getItem('BG Color') !== 'reset' && localStorage.getItem('BG Color') !== '')
    {
